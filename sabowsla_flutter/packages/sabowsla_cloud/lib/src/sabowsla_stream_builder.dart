...
part 'sabowsla_stream_filter_builder.dart';
...
typedef SabowslaStreamEvent = List<Map<String, dynamic>>;

class SabowslaStreamBuilder extends Stream<SabowslaStreamEvent> {
  ...
  BehaviorSubject<SabowslaStreamEvent>? _streamController;

  /// Contains the combined data of postgrest and realtime to emit as stream.
  SabowslaStreamEvent _streamData = [];
  ...
  ///
  /// ```dart
  /// sabowsla.from('users').stream(primaryKey: ['id']).order('username', ascending: false);
  /// ```
  SabowslaStreamBuilder order(String column, {bool ascending = false}) {
  ...
  ///
  /// ```dart
  /// sabowsla.from('users').stream(primaryKey: ['id']).limit(10);
  /// ```
  SabowslaStreamBuilder limit(int count) {
    _limit = count;
    return this;
  }

  @Deprecated('Directly listen without execute instead. Deprecated in 1.0.0')
  Stream<SabowslaStreamEvent> execute() {
    _setupStream();
    return _streamController!.stream;
  }

  @override
  StreamSubscription<SabowslaStreamEvent> listen(
    void Function(SabowslaStreamEvent event)? onData, {
    Function? onError,
  ...
  Future<void> _getStreamData() async {
    ...
    try {
      ...
      final rows = SabowslaStreamEvent.from(data as List);
  ...
  @override
  Stream<E> asyncMap<E>(
      FutureOr<E> Function(SabowslaStreamEvent event) convert) {
    // Copied from [Stream.asyncMap]

    final controller = BehaviorSubject<E>();

    controller.onListen = () {
      StreamSubscription<SabowslaStreamEvent> subscription = listen(null,
      ...
      subscription.onData((SabowslaStreamEvent event) {
        FutureOr<E> newValue;
        try {
  ...
  Stream<E> asyncExpand<E>(
      Stream<E>? Function(SabowslaStreamEvent event) convert) {
