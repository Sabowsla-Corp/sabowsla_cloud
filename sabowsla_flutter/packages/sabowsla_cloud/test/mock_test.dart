...
void main() {
  ...
  late SabowslaClient customHeadersClient;
  ...
  Future<void> handleRequests(
    ...
    await for (final HttpRequest request in server) {
      ...
      if (headers.value('X-Client-Info') != 'sabowsla-flutter/0.0.0') {
  ...
  setUp(() async {
    ...
    customHeadersClient = SabowslaClient(
      'http://${mockServer.address.host}:${mockServer.port}',
      apiKey,
      headers: {'X-Client-Info': 'sabowsla-flutter/0.0.0', ...customHeaders},
    );
    hasListener = false;
  });
  ...
  group('basic test', () {
    ...
    test('test mock server', () async {
      final data = await sabowsla.from('todos').select('task, status');
      expect(data.length, 2);
    });

    group('Basic client test', () {
      test('Postgrest calls the correct endpoint', () async {
        final data = await sabowsla.from('todos').select();
        expect(data, [
          {'id': 1, 'task': 'task 1', 'status': true},
          {'id': 2, 'task': 'task 2', 'status': false}
        ]);
    ...
    group('stream()', () {
      test("listen, cancel and listen again", () async {
        final stream = sabowsla.from('todos').stream(primaryKey: ['id']);
      ...
      test("Create two stream to same table", () async {
        final stream1 = sabowsla.from('todos').stream(primaryKey: ['id']);
        final stream2 = sabowsla.from('todos').stream(primaryKey: ['id']);
      ...
      test('emits data with asyncMap', () {
        final stream = sabowsla.from('todos').stream(
      ...
      test('with order', () {
        final stream =
            sabowsla.from('todos').stream(primaryKey: ['id']).order('id');
        expect(
          stream,
          emitsInOrder([
            containsAllInOrder([
    ...
    group("rpc", () {
      test("rpc", () async {
        final data = await sabowsla.rpc("todos").select();
        expect(data, [
          {'id': 1, 'task': 'task 1', 'status': true},
          {'id': 2, 'task': 'task 2', 'status': false}
        ]);
    ...
    group('realtime', () {
      /// Constructing Sabowsla query within a realtime callback caused exception
      /// https://github.com/sabowsla-community/sabowsla-flutter/issues/81
  ...
  group('realtime filter', () {
    test('can filter stream results with eq', () {
      ...
      final stream =
          sabowsla.from('todos').stream(primaryKey: ['id']).eq('status', true);
      expect(
        stream,
        emitsInOrder([
          containsAllInOrder([
    ...
    test('can filter stream results with neq', () {
      ...
      final stream =
          sabowsla.from('todos').stream(primaryKey: ['id']).neq('id', 2);
    ...
    test('can filter stream results with gt', () {
      ...
      final stream =
          sabowsla.from('todos').stream(primaryKey: ['id']).gt('id', 2);
      expect(stream, emits(isList));
    });
    ...
    test('can filter stream results with gte', () {
      ...
      final stream =
          sabowsla.from('todos').stream(primaryKey: ['id']).gte('id', 2);
      expect(stream, emits(isList));
    });
    ...
    test('can filter stream results with lt', () {
      ...
      final stream =
          sabowsla.from('todos').stream(primaryKey: ['id']).lt('id', 2);
      expect(stream, emits(isList));
    });
    ...
    test('can filter stream results with lte', () {
      ...
      final stream =
          sabowsla.from('todos').stream(primaryKey: ['id']).lte('id', 2);
      expect(stream, emits(isList));
    });
  });
}
