...
import 'package:sabowsla_auth/src/helper.dart';
import 'package:sabowsla_auth/src/sabowsla_auth_admin_api.dart';
import 'package:sabowsla_auth/src/types/auth_response.dart';
import 'package:sabowsla_auth/src/types/fetch_options.dart';
import 'package:sabowsla_auth/src/types/sabowsla_auth_async_storage.dart';

part 'sabowsla_auth_mfa_api.dart';

/// {@template sabowsla_auth}
/// API client to interact with Sabowsla Auth server.
///
/// [url] URL of Sabowsla Auth instance
///
...
/// {@endtemplate}
class SabowslaAuth {
  /// Namespace for the SabowslaAuth API methods.
  /// These can be used for example to get a user from a JWT in a server environment or reset a user's password.
  late final SabowslaAuthAdminApi admin;

  /// Namespace for the SabowslaAuth MFA API methods.
  late final SabowslaAuthMFAApi mfa;
  ...
  final SabowslaAuthAsyncStorage? _asyncStorage;
  ...
  SabowslaAuth({
    ...
    Client? httpClient,
    SabowslaAuthAsyncStorage? asyncStorage,
    AuthFlowType flowType = AuthFlowType.pkce,
  })  : _url = url ?? Constants.defaultSabowslaAuthUrl,
        _headers = headers ?? {},
    ...
    final sabowslaAuthUrl = url ?? Constants.defaultSabowslaAuthUrl;
    final sabowslaAuthHeader = {
      ...Constants.defaultHeaders,
      if (headers != null) ...headers,
    };
    admin = SabowslaAuthAdminApi(
      sabowslaAuthUrl,
      headers: sabowslaAuthHeader,
      ...
      response = await _fetch.request(
        ...
        options: SabowslaAuthRequestOptions(
          ...
          body: {
            ...
            'password': password,
            'data': data,
            'sabowsla_auth_meta_security': {'captcha_token': captchaToken},
  ...
  Future<AuthResponse> verifyOTP({
    ...
    final body = {
      ...
      'sabowsla_auth_meta_security': {'captchaToken': captchaToken},
  ...
  Future<void> reauthenticate() async {
    ...
    final options =
        SabowslaAuthRequestOptions(headers: headers, jwt: session.accessToken);
  ...
  Future<void> setInitialSession(String jsonStr) async {
    ...
    if (session == null) {
      // sign out to delete the local storage from sabowsla_flutter
      await signOut();
