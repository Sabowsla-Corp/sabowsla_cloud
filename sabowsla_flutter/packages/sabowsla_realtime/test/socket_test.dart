...
import 'package:sabowsla_realtime/src/constants.dart';
import 'package:sabowsla_realtime/src/message.dart';
...
void main() {
  ...
  group('constructor', () {
    test('sets defaults', () async {
      final socket = SabowslaRealtime('wss://example.com/socket',
          params: {'apikey': '123'});
    ...
    test('overrides some defaults with options', () async {
      final socket = SabowslaRealtime(
        ...
        headers: {'X-Client-Info': 'sabowsla-dart/0.0.0'},
      );
      expect(socket.channels.length, 0);
      expect(socket.sendBuffer.length, 0);
      expect(socket.ref, 0);
      ...
      expect(socket.headers['X-Client-Info'], 'sabowsla-dart/0.0.0');
    });
  });

  group('endpointURL', () {
    test('returns endpoint for given full url', () {
      final socket = SabowslaRealtime('wss://example.org/chat');
      expect(
        socket.endPointURL,
    ...
    test('returns endpoint with parameters', () {
      final socket =
          SabowslaRealtime('ws://example.org/chat', params: {'foo': 'bar'});
      expect(
        socket.endPointURL,
  ...
  group('connect with Websocket', () {
    late SabowslaRealtime socket;

    setUp(() {
      socket = SabowslaRealtime('ws://localhost:${mockServer.port}');
    });
    ...
    test('sets callback for errors', () {
      dynamic lastErr;
      final SabowslaRealtime erroneousSocket = SabowslaRealtime('badurl')
        ..onError((e) {
          lastErr = e;
        });
  ...
  group('disconnect', () {
    ...
    test('calls connection close callback', () {
      ...
      final mockedSocket = SabowslaRealtime(
        socketEndpoint,
  ...
  group('channel', () {
    ...
    late SabowslaRealtime socket;
    setUp(() {
      socket = SabowslaRealtime(socketEndpoint);
    });
