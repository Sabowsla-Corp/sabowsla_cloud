...
void main() {
  ...
  test('channel should be initially closed', () {
    final channel = RealtimeChannel('topic', SabowslaRealtime('endpoint'));
  ...
  group('constructor', () {
    setUp(() {
      socket =
          SabowslaRealtime('', timeout: const Duration(milliseconds: 1234));
      channel =
  ...
  group('join', () {
    setUp(() {
      socket = SabowslaRealtime('wss://example.com/socket');
  ...
  group('onError', () {
    setUp(() {
      socket = SabowslaRealtime('/socket');
      channel = socket.channel('topic');
      channel.subscribe();
    });
  ...
  group('on', () {
    ...
    setUp(() {
      channel = RealtimeChannel('topic', SabowslaRealtime('endpoint'));
    });
  ...
  group('send', () {
    ...
    test(
      ...
      await for (final HttpRequest req in mockServer) {
        ...
        expect(req.headers.value('apikey'), 'sabowslaKey');
