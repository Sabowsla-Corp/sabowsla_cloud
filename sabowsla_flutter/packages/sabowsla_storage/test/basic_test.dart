...
const String sabowslaUrl = 'SUPABASE_TEST_URL';
const String sabowslaKey = 'SUPABASE_TEST_KEY';
...
String get bucketUrl => '$sabowslaUrl/storage/v1/bucket';
String get objectUrl => '$sabowslaUrl/storage/v1/object';

void main() {
  late SabowslaStorageClient client;
  late CustomHttpClient customHttpClient = CustomHttpClient();

  group('Client with default http client', () {
    setUp(() {
      // init SabowslaClient with test url & test key
      client = SabowslaStorageClient(
  ...
  group('header', () {
    ...
    test('X-Client-Info header can be overridden', () {
      client = SabowslaStorageClient('$sabowslaUrl/storage/v1', {
        'Authorization': 'Bearer $sabowslaKey',
        'X-Client-Info': 'sabowsla-dart/0.0.0'
      });

      expect(client.headers['X-Client-Info'], 'sabowsla-dart/0.0.0');
    });
  });
}
