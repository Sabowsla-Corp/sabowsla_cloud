...
class UsersPageViewUsersList extends ConsumerStatefulWidget {
  const UsersPageViewUsersList({super.key});

  @override
  ConsumerState<UsersPageViewUsersList> createState() =>
      _UsersPageViewUsersListState();
}

class _UsersPageViewUsersListState
    extends ConsumerState<UsersPageViewUsersList> {
  int currentPage = 1;
  int totalPages = 3;
  int usersPerPage = 50;
  ...
  Widget build(BuildContext context) {
    var displayedUsers = ref.watch(authPageControllerProvider).displayedUsers;
    var segmentUsers = displayedUsers.take(usersPerPage).toList();
